 ...
 *      CyclicBarrier。同步辅助类，栅栏。barrier.await();
 *      Semaphore。计数信号量。semp.acquire();semp.release();
 ...
 * （15）线程池Executors。ExecutorService exec = Executors.newCachedThreadPool();
 *      pool.execute(new RunnableTest("Task2"));
 *      Future future = pool.submit(new RunnableTest("Task2"));有返回值
